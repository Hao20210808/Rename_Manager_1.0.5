...
from rename_logic import rename_files, undo_rename
...
def main_app():
    ...
    def update_language(new_lang):
        ...
        btn_undo.config(text=text["undo"])
        ...
        current_index = func_choice.current()
        if current_index == -1:
            func_choice.current(0)
        else:
            func_choice.current(current_index)
    ...
    def run_rename():
        ...
        if not folder:
            ...
            return
        rename_files(folder, func_choice.get(), seq_start.get(),
                     find_text.get(), replace_text.get(), prefix_var.get(),
                     text, tree)

    def run_undo():
        folder = folder_var.get()
        undo_rename(folder, text, tree)

    btn_start = ttk.Button(bottom_frame, text=text["start"], command=run_rename)
    btn_start.pack(side="left", padx=5)
    btn_undo = ttk.Button(bottom_frame, text=text["undo"], command=run_undo)
